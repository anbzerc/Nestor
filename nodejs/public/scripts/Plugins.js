const api_base_path = "http://127.0.0.1:5000"
...
async function getPluginsList() {
    const res = await fetch(`${api_base_path}/api/list`)
    ...
    `
        ...
        document.getElementById(name+"-remove").addEventListener("click", (event) => {
            ...
            try {
                ...
                fetch(api_base_path + "/api/plugins/install", {
                        "method": "POST",
                ...
                fetch(api_base_path + "/api/plugins/remove", {
                        "method": "POST",
                        "body": JSON.stringify(element),
                        "headers": {"Content-type": "application/json; charset=UTF-8"}
                })
